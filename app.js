...
var App=(function(){
  ...
  function renderDashboard(){
    ...
    open.forEach(t=>{ var it=el('div',{class:'item'}); var chk=el('input',{type:'checkbox',onchange:()=>{t.done=true; save(); renderDashboard(); notifyLocal('Teendő kész',t.title);}}); var meta=el('div',{},[el('div',{html:'<strong>'+t.title+'</strong>'}), el('div',{class:'muted',html:memberPill(t.assignee)+' • '+new Date(t.created).toLocaleString()})]); it.append(chk,meta); list.append(it); });
  ...
  // Polling alapértelmezett, 10 mp-enként
  function startPolling(){ setInterval(()=>{ pullAllFromServer().catch(()=>{}); }, 10000); }

  async function init(){ load(); await ensureFamily(); await pullAllFromServer(); bindHeader(); renderTabs('dashboard'); switchView('dashboard'); registerSW(); startPolling(); }
  return { init };
})();
